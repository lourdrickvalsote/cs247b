...
export default function SettingsPage() {
  ...
  useEffect(() => {
    if (!loading && settings.id && !draft) {
      const { id: _id, user_id: _uid, ...rest } = settings;
      setDraft(rest);
    }
  }, [loading, settings, draft]);

  const isDirty = useMemo(() => {
    if (!draft || !settings.id) return false;
    return (Object.keys(draft) as SettingsKey[]).some((key) => {
  ...
  const updateDraft = <K extends SettingsKey>(key: K, value: UserSettings[K]) => {
  ...
  return (
    <div className="max-w-lg mx-auto px-5 py-6 pb-24 animate-fade-in">
      ...
      <section className="mb-6 animate-slide-up" style={{ animationDelay: '0ms' }}>
        ...
        <Card>
          <div className="space-y-5">
            <RangeSetting
              label="Work duration"
              value={draft.work_duration_minutes}
              min={10}
              max={90}
              step={5}
              unit="min"
              onChange={(v) => updateDraft('work_duration_minutes', v)}
            />
            <RangeSetting
              label="Short break"
              value={draft.short_break_minutes}
              min={3}
              max={15}
              step={1}
              unit="min"
              onChange={(v) => updateDraft('short_break_minutes', v)}
            />
            <RangeSetting
              label="Long break"
              ...
              step={5}
              unit="min"
              onChange={(v) => updateDraft('long_break_minutes', v)}
            />
            <RangeSetting
              label="Long break every"
              value={draft.sessions_before_long_break}
              min={2}
              max={6}
              step={1}
              unit="sessions"
              description="Take a longer break after this many focus sessions"
              onChange={(v) => updateDraft('sessions_before_long_break', v)}
            />
          </div>
        </Card>
      </section>
        ...
        <Card>
          <div className="space-y-4">
            <ToggleSetting
              ...
              value={draft.auto_start_breaks}
              onChange={(v) => updateDraft('auto_start_breaks', v)}
            />
            ...
            <ToggleSetting
              ...
              value={draft.auto_start_work}
              onChange={(v) => updateDraft('auto_start_work', v)}
            />
          </div>
        </Card>
      </section>
        ...
        <Card>
          <div className="space-y-4">
            <ToggleSetting
              ...
              value={draft.sound_enabled}
              onChange={(v) => updateDraft('sound_enabled', v)}
            />
            <div className="border-t border-powder-100 dark:border-jet-700" />
            <ToggleSetting
              label="Browser notifications"
              description="Get notified even when the tab is in the background"
              value={draft.notification_enabled}
              onChange={async (v) => {
                if (v) {
      ...
      <section className="mb-6 animate-slide-up" style={{ animationDelay: '240ms' }}>
        ...
        <Card>
          <p className="text-xs text-lilac-500 mb-3">Choose which activity categories to include in your breaks.</p>
          <div className="space-y-2">
            {([
              ...
              const hiddenCategories = draft.hidden_categories ?? [];
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    updateDraft('hidden_categories', next);
                  }}
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border transition-all duration-200 text-left ${
                    included
                      ? 'bg-forest-50/50 border-forest/20 dark:bg-forest-950/10 dark:border-forest/10'
                      : 'bg-white dark:bg-jet-800 border-powder-200 dark:border-jet-700 opacity-60'
                  }`}
                >
                  <CatIcon className={`w-4 h-4 ${included ? 'text-forest' : 'text-lilac-400'}`} />
                  <span className={`text-sm font-medium flex-1 ${included ? 'text-jet dark:text-jet-100' : 'text-jet-500'}`}>
                    {label}
                  </span>
                  <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-all ${
                    included ? 'bg-forest border-forest' : 'border-powder-300 dark:border-jet-600'
                  }`}>
                    {included && (
      ...
      {/* Sticky save button */}
      {isDirty && (
        <div className="fixed bottom-20 left-0 right-0 z-20 flex justify-center animate-slide-up">
          <Button
            size="lg"
            onClick={handleSave}
            disabled={saving}
