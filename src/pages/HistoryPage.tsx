...
import { Clock, Trophy, Coffee, Sparkles, ChevronLeft, ChevronRight, Calendar, Filter, ArrowUpDown } from 'lucide-react';
...
export default function HistoryPage() {
  ...
  return (
    <div className="max-w-lg mx-auto px-5 py-6 pb-24 animate-fade-in">
      ...
      <div className={`grid gap-3 mb-6 ${todayAvgRestoration !== null ? 'grid-cols-4' : 'grid-cols-3'}`}>
        {[
      ...
      {avgTirednessWithBreak !== null && avgTirednessWithoutBreak !== null && (
        <div className="mb-6 animate-slide-up" style={{ animationDelay: '320ms' }}>
          <Card>
            <p className="text-xs font-semibold text-lilac uppercase tracking-wider mb-3">
              Break Impact
            </p>
            <div className="flex gap-4">
              <div className="flex-1 text-center">
                <p className="text-2xl font-bold text-forest">{avgTirednessWithBreak.toFixed(1)}</p>
                <p className="text-[10px] text-lilac-500 font-medium mt-0.5">With break</p>
              </div>
              <div className="w-px bg-powder-200 dark:bg-jet-700" />
              <div className="flex-1 text-center">
                <p className="text-2xl font-bold text-lilac-500">{avgTirednessWithoutBreak.toFixed(1)}</p>
                <p className="text-[10px] text-lilac-500 font-medium mt-0.5">Without break</p>
              </div>
            </div>
            <p className="text-[10px] text-lilac-400 mt-2 text-center">Average energy level (1-5) before starting work</p>
          </Card>
        </div>
      )}
      ...
      <div className="mb-4 space-y-3">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            className="p-1.5 rounded-lg hover:bg-powder-50 dark:hover:bg-jet-800 transition-colors shrink-0"
            title={sortOrder === 'newest' ? 'Sort oldest first' : 'Sort newest first'}
          >
            <ArrowUpDown className="w-3.5 h-3.5 text-lilac-400" />
          </button>
        </div>
      </div>
      ...
      {rounds.length === 0 ? (
        ...
        <>
          <RoundList rounds={sortOrder === 'oldest' ? [...rounds].reverse() : rounds} onSelect={setSelectedRound} />
