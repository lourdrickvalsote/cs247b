    ...
    <div className="space-y-3">
      ...
      {breakSession && (
        <div className="bg-powder-50/50 rounded-xl p-4 border border-powder-200/60">
          ...
          />
          {breakSession.restoration_rating != null && (
            <div className="flex items-center gap-1.5 mt-2">
              <span className="text-[10px] text-lilac-500 font-semibold">Restored</span>
              <div className="flex gap-0.5">
                {[1, 2, 3, 4, 5].map(n => (
                  <div
                    key={n}
                    className={`w-2 h-2 rounded-full ${
                      n <= breakSession.restoration_rating! ? 'bg-forest' : 'bg-powder-200 dark:bg-jet-600'
                    }`}
                  />
                ))}
              </div>
