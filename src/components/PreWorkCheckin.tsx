...
const TIREDNESS_LEVELS = [
  { value: 1, label: 'Exhausted', selectedBg: 'bg-lilac-400' },
  ...
  { value: 5, label: 'Alert', selectedBg: 'bg-forest' },
] as const;

export default function PreWorkCheckin() {
  const { submitPreWorkCheckin, skipPreWorkCheckin } = useSession();
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-5rem)] px-6">
      <div className="max-w-sm w-full text-center">
        <div className="w-16 h-16 bg-powder-50 rounded-2xl flex items-center justify-center mx-auto mb-5 animate-bounce-in">
          <Coffee className="w-8 h-8 text-powder-600" />
        </div>
        ...
        {step === 1 && (
          <>
            ...
            >
              Quick check before your next focus session
            </p>
        ...
        {step === 2 && (
          <>
            ...
            <div className="flex items-start justify-center gap-3 mb-8">
              {TIREDNESS_LEVELS.map((r, i) => {
                const isSelected = tiredness === r.value;
                return (
                  <button
                    key={r.value}
                    onClick={() => setTiredness(r.value)}
            ...
            >
              <Button
                fullWidth
                size="lg"
                disabled={tiredness === null}
                onClick={() => tiredness !== null && tookBreak !== null && submitPreWorkCheckin(tookBreak, tiredness)}
              >
                Continue
              </Button>
            </div>
